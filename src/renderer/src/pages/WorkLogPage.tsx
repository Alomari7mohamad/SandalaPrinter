  ...
  return <div className="page work-log-page">
    ...
    <div className="work-log-top-grid">
      ...
      <section className="work-summary-column">
        ...
        <div className="work-summary-grid"><div className="panel"><Clock3 /><span>ساعات بأجر عادي</span><b>{formatNumber(report?.summary.regularHours ?? 0, 0, 2)}</b><small>الأجر: {formatCurrency(report?.summary.regularPay ?? 0)}</small></div><div className="panel overtime"><Sparkles /><span>ساعات بأجر مع زيادة</span><b>{formatNumber(report?.summary.increasedHours ?? 0, 0, 2)}</b><small>الأجر: {formatCurrency(report?.summary.increasedPay ?? 0)}</small></div><div className="panel"><CalendarClock /><span>أيام العمل</span><b>{formatNumber(report?.summary.workDays ?? 0)}</b><small>إجمالي الساعات: {formatNumber(report?.summary.totalHours ?? 0, 0, 2)}</small></div><div className="panel total"><Coins /><span>إجمالي الأجر المستحق</span><b>{formatCurrency(report?.summary.totalPay ?? 0)}</b></div></div>
      </section>
    </div>
