...
const emptyMaterial = () => ({ name: '', unit: 'قطعة', purchaseMode: 'UNIT' as PurchaseMode, initialQuantity: '0', unitCost: '0', supplierIds: [] as string[], reorderPoint: '1', minimumOrderQuantity: '1', unitsPerPackage: '500', packagePrice: '0', packageNotes: '', reorderPackageCount: '1' })
...
export function InventoryPage() {
  ...
  const load = async () => {
    ...
    try {
      const [stock, traders] = await Promise.all([window.desktopApi.inventory.list(), window.desktopApi.shortages.listSuppliers()])
      setItems(stock.filter((item) => item.itemKind === 'RAW_MATERIAL')); setSuppliers(traders)
    } catch (cause) { setError(getArabicError(cause, 'تعذر تحميل المواد الخام.')) }
    finally { setLoading(false) }
  }
  useEffect(() => { void load() }, [])
  const filtered = useMemo(() => { const query = search.trim().toLowerCase(); return items.filter((item) => !query || item.name.toLowerCase().includes(query)) }, [items, search])
  ...
  const save = async () => {
    ...
    try {
      ...
      const saved = dialog.mode === 'SETTINGS'
        ? await window.desktopApi.inventory.updateSettings({ itemId: dialog.item.id, lowStockThreshold: threshold, purchaseCost: effectiveUnitCost, supplierId: supplierIds[0] ?? null, supplierIds, reorderPoint: threshold, minimumOrderQuantity: Number(minimumOrderQuantity), packageEnabled, packageName: packageEnabled ? 'رزمة' : null, unitsPerPackage: packageEnabled ? Number(unitsPerPackage) : null, packagePrice: packageEnabled ? Number(packagePrice) : null, packageNotes: packageEnabled ? (packageNotes.trim() || null) : null, reorderPackageCount: packageEnabled ? Number(reorderPackageCount) : null })
  ...
  const createItem = async () => {
    ...
    try {
      ...
      const saved = await window.desktopApi.inventory.createItem({ name: newItem.name.trim(), sku: null, barcode: null, itemKind: 'RAW_MATERIAL', unit: newItem.unit.trim(), quantity: isPackage ? initial * unitCount : initial, purchaseCost: isPackage && unitCount > 0 ? Number(newItem.packagePrice) / unitCount : Number(newItem.unitCost), supplierId: newItem.supplierIds[0] ?? null, supplierIds: newItem.supplierIds, categoryId: null, reorderPoint: threshold, minimumOrderQuantity: Number(newItem.minimumOrderQuantity), packageEnabled: isPackage, packageName: isPackage ? 'رزمة' : null, unitsPerPackage: isPackage ? unitCount : null, packagePrice: isPackage ? Number(newItem.packagePrice) : null, packageNotes: isPackage ? (newItem.packageNotes.trim() || null) : null, reorderPackageCount: isPackage ? Number(newItem.reorderPackageCount) : null })
      setItems((rows) => [...rows, saved].sort((a, b) => a.name.localeCompare(b.name, 'ar'))); setNewOpen(false); setNewItem(emptyMaterial()); window.dispatchEvent(new Event('sandala:inventory-changed'))
  ...
  return <div className="page inventory-page">
    <PageHeader title="المخزون" subtitle="إدارة المواد الخام والكميات المستهلكة في تنفيذ الخدمات" action={<button className="primary-button" onClick={() => setNewOpen(true)}><Plus size={17} /> إضافة مادة خام</button>} />
    ...
    <section className="panel catalog-panel inventory-panel">
      <div className="inventory-toolbar"><div className="search-field"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ابحث باسم المادة الخام..." /></div><b>المواد الخام</b><span>{formatNumber(filtered.length)} مادة</span></div>
