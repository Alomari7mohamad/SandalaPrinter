...
import type { InventoryItemDto, ServiceCategoryDto, SupplierDto } from '../../../shared/contracts'
...
export function InventoryPage() {
  ...
  const [newItem,setNewItem]=useState({name:'',sku:'',unit:'قطعة',quantity:'0',purchaseCost:'0',supplierId:'',categoryId:'',reorderPoint:'1',minimumOrderQuantity:'1',packageEnabled:false,packageName:'رزمة',unitsPerPackage:'500',packagePrice:'0',packageNotes:'',reorderPackageCount:'1'})
  const [itemToDelete, setItemToDelete] = useState<InventoryItemDto | null>(null)
  const [saving, setSaving] = useState(false)

  const load = async () => {
    setLoading(true); setError('')
    try { const [stock,traders,groups]=await Promise.all([window.desktopApi.inventory.list(),window.desktopApi.shortages.listSuppliers(),window.desktopApi.catalog.listCategories()]);setItems(stock);setSuppliers(traders);setCategories(groups);setNewItem(current=>({...current,categoryId:current.categoryId||groups.find(group=>group.nameAr==='طباعة')?.id||groups[0]?.id||''})) }
  ...
  const createItem=async()=>{setSaving(true);setError('');try{const packageThreshold=Math.max(0,Number(newItem.reorderPackageCount)*Number(newItem.unitsPerPackage)-1);const saved=await window.desktopApi.inventory.createItem({name:newItem.name,sku:newItem.sku.trim()||null,unit:newItem.unit,quantity:newItem.packageEnabled?Number(newItem.quantity)*Number(newItem.unitsPerPackage):Number(newItem.quantity),purchaseCost:Number(newItem.purchaseCost),supplierId:newItem.supplierId,categoryId:newItem.categoryId,reorderPoint:newItem.packageEnabled?packageThreshold:Number(newItem.reorderPoint),minimumOrderQuantity:Number(newItem.minimumOrderQuantity),packageEnabled:newItem.packageEnabled,packageName:newItem.packageEnabled?newItem.packageName:null,unitsPerPackage:newItem.packageEnabled?Number(newItem.unitsPerPackage):null,packagePrice:newItem.packageEnabled?Number(newItem.packagePrice):null,packageNotes:newItem.packageEnabled?(newItem.packageNotes.trim()||null):null,reorderPackageCount:newItem.packageEnabled?Number(newItem.reorderPackageCount):null});setItems(rows=>[...rows,saved].sort((a,b)=>a.name.localeCompare(b.name,'ar')));setNewOpen(false);setNewItem({name:'',sku:'',unit:'قطعة',quantity:'0',purchaseCost:'0',supplierId:'',categoryId:categories.find(group=>group.nameAr==='طباعة')?.id||categories[0]?.id||'',reorderPoint:'1',minimumOrderQuantity:'1',packageEnabled:false,packageName:'رزمة',unitsPerPackage:'500',packagePrice:'0',packageNotes:'',reorderPackageCount:'1'});window.dispatchEvent(new Event('sandala:inventory-changed'));window.dispatchEvent(new Event('sandala:catalog-changed'))}catch(cause){setError(getArabicError(cause,'تعذر إضافة المنتج للمخزون.'))}finally{setSaving(false)}}
