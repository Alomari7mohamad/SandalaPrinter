  ...
  return (
    <div className="page dashboard-page">
      ...
      <section className="dashboard-sales-insights">
        <article className="panel top-requested-card"><Trophy size={28} /><span>الخدمة الأكثر طلبًا هذا الشهر</span>{mostRequested ? <><strong>{mostRequested.serviceName}</strong><small>{mostRequested.categoryName ?? 'بدون تصنيف'} • الكمية المطلوبة {formatNumber(mostRequested.quantity)}</small></> : <><strong>لا توجد طلبات بعد</strong><small>ستظهر الخدمة الأولى عند تسجيل طلبات هذا الشهر.</small></>}</article>
        <article className="panel monthly-top-products"><div className="panel-heading"><div><h2>أكثر 5 خدمات مبيعًا</h2><p>الشهر الحالي • مرتبة حسب الكمية المباعة</p></div><ShoppingBag size={21} /></div>{topProducts.length === 0 ? <div className="empty-state"><ShoppingBag size={34} /><b>لا توجد كميات مباعة هذا الشهر</b></div> : <div className="monthly-top-list">{topProducts.map((item, index) => <div key={`${item.categoryName}-${item.serviceName}`}><span>{formatNumber(index + 1)}</span><div><b>{item.serviceName}</b><small>{item.categoryName ?? 'بدون تصنيف'}</small></div><strong>{formatNumber(item.quantity)} <small>كمية مباعة</small></strong></div>)}</div>}</article>
      </section>
    </div>
  )
}
