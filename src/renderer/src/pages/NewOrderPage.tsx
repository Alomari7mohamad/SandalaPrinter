        ...
        <section className="panel service-composer">
          ...
          {loading ? <div className="table-state"><LoaderCircle className="spin" size={27} /> جارٍ تحميل الخدمات...</div> : services.length === 0 ? <div className="table-state">لا توجد خدمات نشطة.</div> : <div className="composer-body">
            <div className="service-hierarchy-grid">
              ...
              <label><span><b>3</b> الخدمة</span><select name="serviceProduct" value={selectedId} disabled={!effectiveSelectedType} onChange={(event) => selectService(event.target.value)}><option value="" disabled hidden>{effectiveSelectedType ? 'اختر الخدمة' : 'اختر الصنف أولاً'}</option>{servicesForType.map((service) => <option key={service.id} value={service.id}>{service.nameAr}</option>)}</select></label>
