...
const empty: ServiceInput = { code: '', nameAr: '', nameHe: null, categoryId: '', paperType: null, size: null, colorMode: null, coverage: null, unit: 'قطعة', costType: 'PER_UNIT', unitCost: null, costBatchSize: null, active: true, notes: null }
const toInput = (service?: ServiceDto): ServiceInput => service ? { id: service.id, code: service.code, nameAr: service.nameAr, nameHe: service.nameHe, categoryId: service.categoryId ?? '', paperType: service.paperType, size: service.size, colorMode: service.colorMode, coverage: service.coverage, unit: service.unit, costType: 'PER_UNIT', unitCost: service.costType === 'PER_100' && service.unitCost !== null ? service.unitCost / (service.costBatchSize ?? 100) : service.unitCost, costBatchSize: null, active: service.active, notes: service.notes } : empty
...
export function ServiceDialog({ service, pricingRules, categories, onClose, onSaved }: { service?: ServiceDto; pricingRules: PriceRule[]; categories: ServiceCategoryDto[]; onClose: () => void; onSaved: (service: ServiceDto) => void }) {
  ...
  return <Modal title={service ? 'تعديل الخدمة' : 'خدمة جديدة'} onClose={onClose} wide>
    <form onSubmit={submit} className="dialog-form">
      {error && <div className="alert error form-span">{error}</div>}
      <label>اسم الخدمة بالعربية<input required dir="rtl" value={form.nameAr} onChange={(e) => setForm({ ...form, nameAr: e.target.value })} /></label>
      <label>שם השירות בעברית<input dir="rtl" value={form.nameHe ?? ''} onChange={(e) => text('nameHe', e.target.value)} placeholder="اختياري" /></label>
      <label>رمز الخدمة<input required dir="ltr" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase().replace(/\s+/g, '_') })} /></label>
      ...
      <label className="checkbox-label form-span"><input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} /> الخدمة نشطة</label>
      <div className="dialog-actions form-span"><button type="button" className="secondary-button" onClick={onClose}>إلغاء</button><button className="primary-button" disabled={saving}>{saving ? 'جارٍ الحفظ...' : 'حفظ الخدمة'}</button></div>
    </form>
  </Modal>
}
