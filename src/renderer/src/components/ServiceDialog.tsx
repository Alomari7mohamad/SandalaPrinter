...
import type { InventoryItemDto, MaterialRequirementInput, PricingRuleInput, RawMaterialCategoryDto, ServiceCategoryDto, ServiceDto, ServiceInput } from '../../../shared/contracts'
...
const toInput = (service?: ServiceDto): ServiceInput => service ? { id: service.id, code: service.code, nameAr: service.nameAr, nameHe: service.nameHe, categoryId: service.categoryId ?? '', paperType: service.paperType, size: service.size, colorMode: service.colorMode, coverage: service.coverage, unit: service.unit, itemType: 'SERVICE', supplierId: null, reorderPoint: service.reorderPoint, minimumOrderQuantity: service.minimumOrderQuantity, costType: 'PER_UNIT', unitCost: service.costType === 'PER_100' && service.unitCost !== null ? service.unitCost / (service.costBatchSize ?? 100) : service.unitCost, costBatchSize: null, active: service.active, notes: service.notes } : empty
...
export function ServiceDialog({ service, pricingRules, categories, onClose, onSaved }: { service?: ServiceDto; pricingRules: PriceRule[]; categories: ServiceCategoryDto[]; onClose: () => void; onSaved: (service: ServiceDto) => void }) {
  ...
  const [materialCategories,setMaterialCategories]=useState<RawMaterialCategoryDto[]>([])
  const [materialCategoryId,setMaterialCategoryId]=useState('')
  const [requirements,setRequirements]=useState<MaterialRequirementInput[]>([])
  useEffect(() => {
    void Promise.all([window.desktopApi.inventory.list(),window.desktopApi.inventory.listCategories(),service ? window.desktopApi.catalog.listMaterialRequirements(service.id) : Promise.resolve([])])
      .then(([inventoryRows,categoryRows,recipe])=>{setMaterials(inventoryRows);setMaterialCategories(categoryRows);setRequirements(recipe.map((item)=>({inventoryItemId:item.inventoryItemId,quantityPerUnit:item.quantityPerUnit})))})
      .catch(()=>{setMaterials([]);setMaterialCategories([])})
  }, [service?.id])
  const text = (key: keyof ServiceInput, value: string) => setForm((current) => ({ ...current, [key]: value.trim() === '' ? null : value }))
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError(''); setSaving(true)
    try {
      const saved = await window.desktopApi.catalog.saveService({ ...form, itemType: 'SERVICE', supplierId: null, costType: 'PER_UNIT', costBatchSize: null })
  ...
  const availableMaterials=materials.filter((material)=>!requirements.some((item)=>item.inventoryItemId===material.id)&&(materialCategoryId==='uncategorized'?!material.rawMaterialCategoryId:material.rawMaterialCategoryId===materialCategoryId))
  const hasUncategorizedMaterials=materials.some((material)=>!material.rawMaterialCategoryId)
  return <Modal title={service ? 'تعديل الخدمة' : 'إضافة خدمة'} onClose={onClose} wide>
    <form onSubmit={submit} className="dialog-form">
      ...
      <label>تكلفة تنفيذ الخدمة للوحدة<input type="number" min="0" step="0.001" value={form.unitCost ?? ''} onChange={(e) => setForm({ ...form, unitCost: e.target.value === '' ? null : Number(e.target.value) })} placeholder={`تكلفة تنفيذ ${form.unit || 'الوحدة'} الواحدة`} /></label>
      {!hasComplexPricing && <label>سعر بيع الخدمة للوحدة<input type="number" min="0" step="0.001" value={salePrice ?? ''} onChange={(e) => setSalePrice(e.target.value === '' ? null : Number(e.target.value))} placeholder="اختياري" /></label>}
      ...
      <section className="material-recipe form-span"><div className="material-recipe-picker"><select value={materialCategoryId} onChange={(e)=>setMaterialCategoryId(e.target.value)}><option value="" disabled>تصنيف المواد الخام</option>{materialCategories.map((category)=><option key={category.id} value={category.id}>{category.nameAr}</option>)}{hasUncategorizedMaterials&&<option value="uncategorized">غير مصنف</option>}</select><select value="" disabled={!materialCategoryId} onChange={(e)=>addMaterial(e.target.value)}><option value="">{materialCategoryId?'اختر مادة خام':'اختر التصنيف أولاً'}</option>{availableMaterials.map((material)=><option key={material.id} value={material.id}>{material.name}</option>)}</select></div>{requirements.length>0&&<div className="recipe-list">{requirements.map((requirement)=>{const material=materials.find((item)=>item.id===requirement.inventoryItemId);return <div className="recipe-row" key={requirement.inventoryItemId}><span><b>{material?.name ?? 'مادة خام'}</b></span><label>الكمية<input type="number" inputMode="decimal" min="0.000001" step="any" value={requirement.quantityPerUnit} onChange={(e)=>setRequirements((current)=>current.map((item)=>item.inventoryItemId===requirement.inventoryItemId?{...item,quantityPerUnit:Number(e.target.value)}:item))}/><em>{material?.unit ?? ''}</em></label><button type="button" className="danger-icon" onClick={()=>setRequirements((current)=>current.filter((item)=>item.inventoryItemId!==requirement.inventoryItemId))}>حذف</button></div>})}</div>}</section>
      ...
      <div className="dialog-actions form-span"><button type="button" className="secondary-button" onClick={onClose}>إلغاء</button><button className="primary-button" disabled={saving}>{saving ? 'جارٍ الحفظ...' : 'حفظ التغييرات'}</button></div>
    </form>
  </Modal>
}
