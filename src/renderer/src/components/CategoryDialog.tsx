...
export function CategoryDialog({ onClose, onSaved }: { onClose: () => void; onSaved: (category: ServiceCategoryDto) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const saved = await window.desktopApi.catalog.saveCategory({ nameAr: name.trim() })
      window.dispatchEvent(new Event('sandala:catalog-changed'))
      onSaved(saved)
    } catch (cause) { setError(getArabicError(cause, 'تعذر حفظ التصنيف.')) }
    finally { setSaving(false) }
  }
  return <Modal title="إضافة تصنيف" onClose={onClose}>
    <form className="dialog-form one-column" onSubmit={submit}>
      ...
      <div className="dialog-actions"><button type="button" className="secondary-button" onClick={onClose}>إلغاء</button><button className="primary-button" disabled={saving || name.trim().length < 2}>{saving ? 'جارٍ الحفظ...' : 'إضافة التصنيف'}</button></div>
    </form>
  </Modal>
}
