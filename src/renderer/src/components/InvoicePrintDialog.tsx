    ...
    <article className="modal wide invoice-dialog" role="dialog" aria-modal="true" aria-label="طباعة الطلب">
      ...
      <div className="modal-body invoice-modal-body">
        ...
        <section className={`invoice-print-sheet invoice-size-${preferences.pageSize.toLowerCase()}`} dir="rtl" lang={preferences.language}>
          ...
          <section className="invoice-summary">{order.discountAmount > 0 && <><div><span>{text.subtotal}</span><b dir="ltr">{money(order.subtotal)}</b></div><div><span>{text.discount}</span><b dir="ltr">- {money(order.discountAmount)}</b></div></>}<div><span>{text.beforeTax}</span><b dir="ltr">{money(totals.beforeTax)}</b></div><div><span>{text.tax}</span><b dir="ltr">{money(totals.tax)}</b></div><div className="invoice-final-total"><span>{text.final}</span><b dir="ltr">{money(totals.total)}</b></div><div><span>{text.payment}</span><b>{order.paymentStatus === 'PAID' ? text.paid : text.unpaid}</b></div></section>
