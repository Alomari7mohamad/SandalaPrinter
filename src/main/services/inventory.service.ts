...
const itemSchema=z.object({ name:z.string().trim().min(2).max(150), sku:z.string().trim().max(80).nullable(), unit:z.string().trim().min(1).max(40), quantity:z.number().min(0).finite(), purchaseCost:z.number().min(0).finite(), supplierId:z.string().min(2), categoryId:z.string().min(2), reorderPoint:z.number().min(0).finite(), minimumOrderQuantity:z.number().positive().finite(), ...packageFields }).superRefine((value, context) => { if(value.packageEnabled && (!value.packageName || !value.unitsPerPackage || value.packagePrice===null || value.reorderPackageCount===null)) context.addIssue({code:'custom',message:'أكمل بيانات العبوة.'}) })
